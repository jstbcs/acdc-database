...
condition_df <- readxl::read_excel("./Create_db/add_data/Book.xlsx", "condition_descriptives", range = "A25:F33",
                                   col_names = c("study_in_publication",
                                                 "dataset & condition",	"percentage_congr",
                                                 "percentage_neutral",	"mean_obs_pp",	"n_obs"))

# NOTE: read in datasets 35-40, as well as datasets 46-48
...
for(i in 1:nrow(study_df)){ # within each study
  ...
  for(k in 1:study_df$n_data[i]){ # loop over each dataset
    ...
    pub[[i+1]][[k+2]]$dataset_table <- data.frame(
      ...
      dataset_comment = "Number of trials per block and participant can vary since failed trials were repeated at the end of each block. Participants will a trial count above 200 per block were removed from this dataset.", 
      between = NA,
...
# save list object -------------------------------------------------------------
saveRDS(pub, file="./Create_db/add_data/whitehead_list.RData")
